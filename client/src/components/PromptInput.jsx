...
import {ArrowRightIcon, CloudUploadIcon, Loader2, Loader2Icon, MicIcon} from "lucide-react"
const PromptInput = ({onSubmit , loading = false , placeholder="Describe the website you want to build..." , large=false , autoFocus=false , varient = "default"}) => {
    ...
    if (varient === "glass") {
        return (
            <form onSubmit={handleSubmmit} className="max-w-2xl w-full bg-white/10 backdrop-blur-xl rounded-xl ring-1 ring-white/25 focus-within:ring-2 focus-within:ring-white/30 overflow-hidden mt-6 transition">
                <textarea
                    ref={textareaRef}
                    value={value}
                    ...
                    disabled={loading}
                    rows={3}
                    className='w-full p-4 resize-none placeholder:text-white/60 outline-none bg-transparent text-white text-base'
                />
  ...
  return (
    <div
        ...
        <textarea
            ...
            disabled={loading}
            rows={large ? 5 : 1}
            className={`flex-1 outline-none resize-none text-zinc-900 ${large ? "text-base" : "text-sm"}`}
        />
